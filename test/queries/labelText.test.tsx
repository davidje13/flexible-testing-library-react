...
describe('labelText', () => {
  ...
  it('shows a helpful error if no items are found', () => {
    render(<Component />);
    expect(() => screen.getAllBy(labelText('Nope'))).toThrow(
      'Unable to find a label with the text of: Nope',
    );
  });

  it('shows a helpful error if a label without an input is found', () => {
    render(<Component />);
    expect(() => screen.getAllBy(labelText('Baz'))).toThrow(
      'Found a label with the text of: Baz, however no form control was found associated to that label.',
    );
  });
});
